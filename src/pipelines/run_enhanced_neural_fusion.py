...
from src.models.enhanced_neural_fusion_regressor import (
    ...
    run_tabular_expected_gradients,
)
...
def _load_processed_splits(cfg: dict[str, Any]) -> tuple[pd.DataFrame, pd.DataFrame | None, list[str]]:
    ...
    if dev_path.exists() and dev_diag_path.exists():
        dev_df, _ = _load_one_processed_split(
            feature_dir=feature_dir,
...
def _resolve_training_config(cfg: dict[str, Any]) -> TrainingConfig:
    ...
    return TrainingConfig(
        ...
        loss_name=str(training.get("loss_name", "huber")).strip().lower(),
        huber_delta=float(training.get("huber_delta", 1.0)),
        smooth_l1_beta=float(training.get("smooth_l1_beta", 1.0)),
        seed=random_seed,
        device=str(training.get("device", "cuda" if torch.cuda.is_available() else "cpu")),
    )

def _resolve_explainability_config(cfg: dict[str, Any]) -> dict[str, Any]:
    explainability = cfg.get("explainability") if isinstance(cfg.get("explainability"), dict) else {}

    background_rows = explainability.get("shap_background_max_rows", 64)
    explain_rows = explainability.get("shap_explain_max_rows", None)
    ...
    return {
        "run_shap": bool(explainability.get("run_shap", True)),
        "shap_explain_max_rows": explain_rows,       # None = full dev set
        "shap_background_max_rows": background_rows,
        "shap_max_display": int(explainability.get("shap_max_display", 20)),
        "shap_dependence_top_k": int(explainability.get("shap_dependence_top_k", 8)),
    }

def _resolve_loss_function(training_cfg: TrainingConfig) -> nn.Module:
    if training_cfg.loss_name == "mse":
        return nn.MSELoss()

    if training_cfg.loss_name == "huber":
        return nn.HuberLoss(delta=training_cfg.huber_delta)

    if training_cfg.loss_name == "smooth_l1":
        try:
            return nn.SmoothL1Loss(beta=training_cfg.smooth_l1_beta)
        except TypeError:
            return nn.SmoothL1Loss()

    raise ExperimentRuntimeError(f"Unsupported loss_name '{training_cfg.loss_name}'.")
...
def _resolve_model_config(
    ...
    *,
    ...
    return EnhancedNeuralFusionRegressorConfig(
        encoder_name=encoder_name,
        tabular_dim=tabular_dim,
        text_pooling=str(params.get("text_pooling", "attention")),
        tabular_hidden_dim=int(params.get("tabular_hidden_dim", 256)),
        shared_hidden_dim=int(params.get("shared_hidden_dim", 256)),
        fusion_hidden_dim=int(params.get("fusion_hidden_dim", 256)),
        tabular_num_layers=int(params.get("tabular_num_layers", 3)),
        ...
        tabular_use_residual=bool(params.get("tabular_use_residual", True)),
        use_gated_fusion=bool(params.get("use_gated_fusion", True)),
        multi_sample_dropout_samples=int(params.get("multi_sample_dropout_samples", 5)),
    )
...
def _prepare_inference_dataframe(
    ...
    *,
    ...
    has_target = target_column in df.columns and df[target_column].notna().any()
    if has_target:
        return df.copy(), True
    ...
    if target_column not in out_df.columns:
        ...
        out_df[target_column] = out_df[target_column].fillna(0.0)
    return out_df, False

def _slice_value(value: Any, take: int) -> Any:
    if torch.is_tensor(value):
        return value[:take].detach().cpu()
...
def collect_explain_batches(
    ...
    *,
    ...
    tabular_parts: list[torch.Tensor] = []
    item_ids: list[str] = []
    ...
    for batch in loader:
        ...
        if max_rows is None:
            ...
            remaining = max_rows - n_collected
            if remaining <= 0:
                break
            take = min(batch_size, remaining)

        input_ids_parts.append(_slice_value(batch["input_ids"], take))
        attention_mask_parts.append(_slice_value(batch["attention_mask"], take))
        tabular_parts.append(_slice_value(batch["tabular_features"], take))

        if "item_id" in batch:
            batch_ids = _slice_value(batch["item_id"], take)
            if torch.is_tensor(batch_ids):
                batch_ids = batch_ids.tolist()
            item_ids.extend([str(x) for x in batch_ids])

        n_collected += take

        if max_rows is not None and n_collected >= max_rows:
            break

    if not input_ids_parts:
        raise ExperimentRuntimeError("No rows were collected for explainability.")

    return {
        "input_ids": torch.cat(input_ids_parts, dim=0),
        "attention_mask": torch.cat(attention_mask_parts, dim=0),
        "tabular_features": torch.cat(tabular_parts, dim=0),
        "item_id": item_ids,
    }
...
def _run_fold_training_and_predict(
    *,
    ...
    resolved_config: dict[str, Any],
    training_cfg: TrainingConfig,
) -> tuple[np.ndarray, np.ndarray | None, dict[str, float], dict[str, Any]]:
    ...
    predict_input_df: pd.DataFrame | None = None
    predict_has_target = False
    predict_dataset = None

    if predict_df is not None and not predict_df.empty:
        predict_input_df, predict_has_target = _prepare_inference_dataframe(
            predict_df,
            target_column=target_column,
        )
        predict_dataset = builder.build_dataset(predict_input_df)
    ...
    model = EnhancedNeuralFusionRegressor(model_cfg).to(training_cfg.device)
    loss_fn = _resolve_loss_function(training_cfg)
    ...
    predict_pred: np.ndarray | None = None
    if predict_loader is not None:
        predict_pred = _predict(
            model=model,
    ...
    training_summary = {
        ...
        "history": history,
        "prediction_only": not predict_has_target if predict_df is not None else True,
    }

    return valid_pred, predict_pred, valid_metrics, training_summary
...
def _write_run_metadata(
    *,
    ...
    payload = {
        ...
        "model_name": "enhanced_neural_fusion",
    ...
    *,
    ...
    checkpoint = {
        ...
        "model_config": {
            ...
            "shared_hidden_dim": model.cfg.shared_hidden_dim,
            ...
            "tabular_use_residual": model.cfg.tabular_use_residual,
            "use_gated_fusion": model.cfg.use_gated_fusion,
            "multi_sample_dropout_samples": model.cfg.multi_sample_dropout_samples,
        },
    }
...
def _save_submission(
    *,
    ...
    if list(reloaded.columns) != [id_column, "prediction"]:
        raise ExperimentRuntimeError(
            f"{filename} must contain exactly columns: {id_column},prediction"
        )


def _save_fold_test_predictions(
    *,
    output_dir: Path,
    ...
    if not fold_predictions:
        raise ExperimentRuntimeError("No fold predictions available to save.")

    pred_matrix = np.column_stack(fold_predictions)
    avg_pred = pred_matrix.mean(axis=1)

    out_df = predict_df[[id_column]].copy()
    for i in range(pred_matrix.shape[1]):
        out_df[f"prediction_fold_{i+1}"] = pred_matrix[:, i]
    out_df["prediction_mean"] = avg_pred
    out_df.to_csv(output_dir / "test_fold_predictions.csv", index=False)
...
def _fit_final_model_and_predict_dev(
    *,
    ...
    explainability_cfg = _resolve_explainability_config(resolved_config)
    output_dir = _resolve_output_dir(resolved_config)
    train_dataset = builder.build_dataset(train_df)
    inference_dev_df, dev_has_target = _prepare_inference_dataframe(
        dev_df,
        target_column=target_column,
    )
    ...
    if explainability_cfg["run_shap"]:
        dev_explain_data = collect_explain_batches(
            dev_loader,
            max_rows=explainability_cfg["shap_explain_max_rows"],   # None = full dev set
        )
        train_bg_data = collect_explain_batches(
            train_bg_loader,
            max_rows=explainability_cfg["shap_background_max_rows"],
        )

        shap_result = run_tabular_expected_gradients(
            model=model,
            explain_tabular=dev_explain_data["tabular_features"],
            background_tabular=train_bg_data["tabular_features"],
            explain_input_ids=dev_explain_data["input_ids"],
            explain_attention_mask=dev_explain_data["attention_mask"],
            feature_names=numeric_features,
            output_dir=output_dir / "expected_gradients_tabular",
            explain_item_ids=dev_explain_data["item_id"] if dev_explain_data["item_id"] else None,
            max_display=explainability_cfg["shap_max_display"],
            dependence_top_k=explainability_cfg["shap_dependence_top_k"],
        )

        shap_importance_df = shap_result["importance_df"]
        shap_summary = {
            "ran_shap": True,
            "n_explained_rows": int(dev_explain_data["tabular_features"].shape[0]),
            "n_background_rows": int(train_bg_data["tabular_features"].shape[0]),
...
def run_neural_fusion_experiment(
    *,
    ...
    predict_df_for_ensemble: pd.DataFrame | None = None
    if dev_df is not None:
        predict_df_for_ensemble = dev_df.copy()
        if target_column in predict_df_for_ensemble.columns:
            non_missing_mask = predict_df_for_ensemble[target_column].notna()
            if non_missing_mask.any():
                predict_df_for_ensemble = predict_df_for_ensemble.loc[non_missing_mask].reset_index(drop=True)
            else:
                predict_df_for_ensemble = predict_df_for_ensemble.drop(columns=[target_column]).reset_index(drop=True)
    ...
    for fold_number, (train_idx, valid_idx) in enumerate(splitter.split(run_df), start=1):
        ...
        y_pred, fold_test_pred, valid_metrics, training_summary = _run_fold_training_and_predict(
            fold_train_df=fold_train_df,
            fold_valid_df=fold_valid_df,
            predict_df=predict_df_for_ensemble,
            text_columns=text_columns,
            numeric_features=numeric_features,
            target_column=target_column,
            metric_names=metric_names,
        ...
        fold_summary_payload = {
            "fold": fold_number,
            **training_summary,
            "valid_metrics": valid_metrics,
        }

        if fold_test_pred is not None:
            fold_test_predictions.append(fold_test_pred)

        fold_training_summaries.append(fold_summary_payload)
    ...
    if (
        predict_df_for_ensemble is not None
        and not predict_df_for_ensemble.empty
        and id_column is not None
        and fold_test_predictions
    ):
        ensemble_pred = _save_fold_test_predictions(
            output_dir=output_dir,
            predict_df=predict_df_for_ensemble,
            id_column=id_column,
            fold_predictions=fold_test_predictions,
        )
        _save_submission(
            output_dir=output_dir,
            df=predict_df_for_ensemble,
            id_column=id_column,
            predictions=ensemble_pred,
            filename="submission_fold_ensemble.csv",
        )
