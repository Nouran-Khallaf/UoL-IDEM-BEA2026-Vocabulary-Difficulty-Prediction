...
from src.pipelines.run_neural_fusion import run_neural_fusion_experiment
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a neural fusion BEA KVL experiment from a resolved/inheriting YAML config. "
...
def _resolve_experiment_type(cfg: dict[str, Any]) -> str:
    exp_type = cfg.get("experiment_type", "neural_fusion")
...
def main() -> None:
    ...
    if experiment_type not in SUPPORTED_EXPERIMENT_TYPES:
        raise ExperimentRuntimeError(
            f"run_neural_fusion_cli.py expected one of {sorted(SUPPORTED_EXPERIMENT_TYPES)}, "
