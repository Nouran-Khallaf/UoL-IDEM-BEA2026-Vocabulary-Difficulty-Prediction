...
from src.pipelines.run_text_feature_regression import (
    run_text_feature_regression_experiment,
)


SUPPORTED_EXPERIMENT_TYPES = {
    "text_feature_regression",
    "prompt_fusion",
    "features_as_text",
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run a prompt-based BEA KVL experiment from a resolved/inheriting YAML config. "
            "This runner is for the architecture where engineered features are rendered as "
            "structured text and appended to the transformer input."
        )
    )
    parser.add_argument(
        "--config",
...
def _resolve_experiment_type(cfg: dict[str, Any]) -> str:
    exp_type = cfg.get("experiment_type", "text_feature_regression")
...
def main() -> None:
    ...
    if experiment_type not in SUPPORTED_EXPERIMENT_TYPES:
        raise ExperimentRuntimeError(
            f"run_text_feature_regression_cli.py expected one of "
            f"{sorted(SUPPORTED_EXPERIMENT_TYPES)}, got '{experiment_type}'."
        )

    result = run_text_feature_regression_experiment(
    ...
    summary = {
        ...
        "prompt_feature_columns": result.prompt_feature_columns,
        "n_prompt_feature_columns": len(result.prompt_feature_columns),
