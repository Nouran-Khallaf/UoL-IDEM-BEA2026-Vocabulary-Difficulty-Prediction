...
class NeuralFusionDatasetConfig:
    ...
    text_columns:
        Text columns to concatenate into one encoder input.
    numeric_feature_columns:
        Numeric engineered feature columns used as the tabular branch.
    target_column:
        Gold regression target column.
    max_length:
        Maximum token length for tokenizer truncation.
    text_join_mode:
        How to join text columns:
        - 'sep'     -> " [SEP] "
        - 'newline' -> "\\n"
        - 'space'   -> " "
    add_special_tokens:
        Whether tokenizer should add model special tokens.
    """
    encoder_name: str
...
def _join_text_parts(parts: Sequence[str], mode: str) -> str:
    if mode == "sep":
...
class TabularFeaturePreprocessor:
    """
    Fit/transform helper for numeric tabular features used in neural fusion.
    ...
    Notes
    -----
    - This preprocessor is intended for dense numeric engineered features.
    - Non-numeric columns should not be passed here.
    - Fit on train only, then reuse for dev/test.
    """
...
class NeuralFusionRegressionDataset(Dataset):
    """
    PyTorch dataset for neural fusion regression.
    ...
    The dataset assumes tabular features are already numerically transformed
    using TabularFeaturePreprocessor.
    """
...
class NeuralFusionDatasetBuilder:
    ...
    2. fit_tabular_preprocessor(train_df)
    3. build_dataset(train_df)
    4. build_dataset(dev_df)

    This ensures tabular scaling is fitted on train only.
    """
    ...
    def _build_texts(self, df: pd.DataFrame) -> list[str]:
        ...
        for _, row in df.iterrows():
            ...
            text = _join_text_parts(parts, self.cfg.text_join_mode).strip()
            texts.append(text)
        return texts

    def fit_tabular_preprocessor(self, train_df: pd.DataFrame) -> None:
        self.tabular_preprocessor.fit(
            train_df,
            self.cfg.numeric_feature_columns,
        )
